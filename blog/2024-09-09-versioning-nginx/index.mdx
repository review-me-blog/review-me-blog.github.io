...
리눅스에서는 `envsubst`라는 함수를 제공합니다. `environment variable substitution`, 환경 변수 치환이예요. 문자열을 제공한 뒤, 리눅스 내 환경 변수가 존재한다면 해당 내용으로 치환합니다. 이때, `${VAR_NAME}`와 같은 형태로 제공해야 해요.
...
전자의 경우 컨테이너를 다시 띄워야 하겠지만, 후자의 경우에는 nginx에게 새로고침 명령만 내리면 됩니다. 다른 말로 하자면 `.conf`만 변경되었을 때 컨테이너가 다시 띄워질 필요가 없다는 의미입니다. 이를 어떻게 판단할 수 있을까요?

우선 workflow가 돌아가기 위해 `/nginx` 아래 파일이 바뀌어야 한다는 전제가 있습니다. 아래 규칙을 지킨다면, 비교적 간단하게 해결할 수 있습니다.
- PR을 통해 Squash and merge를 진행할 것

Squash and merge를 진행하게 되면 변경 이력이 하나의 커밋으로 바뀌게 됩니다. 직전 커밋과 Squash된 현재 커밋을 비교해 `docker-compose.yml`이 존재하는지 찾아보는 것도 하나의 방법입니다. 해당 규칙을 사용하지 않고 다양한 방법을 활용해 변경사항을 찾아낼 수도 있겠지만, 저희가 찾은 효율적인 방법 중 하나라 사용하게 되었어요.
